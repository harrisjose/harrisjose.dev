...
> If you already have an site running on vercel, a cool thing you can do is set it to run under a subdomain. Mine runs at [umami.harrisjose.dev](https://umami.harrisjose.dev). Just enter the subdomain in your settings in vercel and you should be ready to go.

## Creating the tables

To create the tables needed for umami, you'll need to run a few queries. I really recommend [Postico](https://eggerapps.at/postico/) if you dont already have a client that connects to postgres. Connect to your databse and copy paste the queries from `umami/prisma/schema.postgresql.prisma` or the mysql equivalent.
